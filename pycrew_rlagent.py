...
class RLAgent(Agent):
    ...
    def get_action(self, game):
        ...
        # get legal moves (no communication) #TODO: add communication
        moves = self.get_legal_moves(game)
        self.epsilon = 80 - self.n_games
        final_move = [0 for move in moves]
        if random.randint(0, 200) < self.epsilon:
            move = random.randint(0, len(final_move))
            # moves[move] = 1
        else:
            state0 = torch.tensor(self.get_state(game), dtype=torch.float)
